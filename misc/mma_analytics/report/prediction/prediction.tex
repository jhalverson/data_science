...
\clearpage
\subsection*{Simple Model based on the Difference in Cumulative Win Ratios}
...
$\Delta w_{r,i}$ is the difference in cumulative win ratios between fighter 1 and fighter 2 in fight $i$.
...
or standardization.
Our suspicion is that they fit the entire data set and then computed
percent concordant/discordant by using the model to make predictions on the
same data set. 
However, even when we train on all the samples and predict
on those same samples,
we still achieve an accuracy of only 57.3\%.
...
\caption{Accuracy of logistic regression model using
only the difference between the cumulative win ratios of the fighters
...
The accuracy for this case is 56.5\%.
...
of $\alpha$ and $\beta$ are 0 and 0.089, respectively.
